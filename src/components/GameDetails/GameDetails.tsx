    ...
    <div className="game-container">
        ...
        <div className="datee-box">
          ...
          <p className="time">
            {new Date(
              ...
              {convertUnixToLocalTime(bookDetails?.startTime)}-
              {convertUnixToLocalTime(bookDetails?.endTime)}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="players-section">
        <div className="players-header">
          <h3>Players</h3>
          <span
            className={`session-type-1 ${
              gameDetails?.difficultyLevel === "Beginner"
                ? "game-bg-beginner"
                : gameDetails?.difficultyLevel === "Intermediate"
                ? "game-bg-intermediate"
                : gameDetails?.difficultyLevel === "Advanced"
                ? "game-bg-difficult"
                : ""
            }`}
          >
          ...
          {gameDetails?.scheduledPlayersDetails?.map((player, i) => (
            <div key={i} className="player-card">
              {images && images[player.userId] ? (
                ...
                <img src="/default-avatar.png" alt="Player" />
              )}
