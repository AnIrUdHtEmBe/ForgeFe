...
const BookWellness = () => {
  ...
  const clickHandler = (value: t_sport) => {
    navigate(E_Routes.detailedViewWellness, {
      state: {
        descriptor: value,
  ...
  return (
    <div className="book-wellness-container">
      <div className="--back">
        <IoIosArrowBack onClick={backHandler} size={DEFAULT_ICON_SIZE - 5} />
      </div>
      <div className="book-wellness-content-container">
        <div className="book-wellness-top-heading-container">
          <div className="--top-heading">{category == "FITNESS" ? "Fitness" : "Wellness"}</div>
          <div className="--bottom-content">
            {sports.map((el, i) => {
              return (
                <div key={i} className="--wellness-box">
                  <span className="--icon">{wellnessWindows.find(obj => obj.name == el.name)?.icon ?wellnessWindows.find(obj => obj.name == el.name)?.icon  : "string" }</span>
                  <span className="--text">{el.name}</span>
                  <Button text="Explore" onClick={() => clickHandler(el)} />
                </div>
              );
            })}
          </div>
